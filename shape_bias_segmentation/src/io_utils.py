""" Utils for managing experiment results.
"""
import os
import subprocess
...
def flatten_namespace(namespace: Namespace, prev_key: str = "") -> dict:
    """Recursive flattening a Namespace"""
    dct: dict = {}
    for key, value in vars(namespace).items():
        new_key = f"{prev_key}.{key}" if prev_key is not "" else key
        if isinstance(value, Namespace):
            dct.update(flatten_namespace(value, prev_key=new_key))
        else:
            dct[new_key] = value
    return dct
