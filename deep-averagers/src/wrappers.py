...
class TorchWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        if obs.ndim == 3:
            ...
            return obs.view(1, 1, *obs.shape).contiguous()
        return obs.view(1, 1, -1).float().to(self._device).contiguous()
...
def get_env(opt, mode="train", **kwargs):
    ...
    if "stochasticity" not in kwargs:
        kwargs["clip_rewards"] = opt.__dict__.get("clip_rewards", 1)
...
def get_wrapped_atari(
    ...
    return ALE(
        ...
        training=(mode == "train"),
        clip_rewards_val=kwargs.get("clip_rewards", 1),
    )
