...
def dydx_norm_penalty(inputs, outputs, k=1.0, mode="norm_sum"):
    """ Computes the gradient norm penalty.

    Args:
        inputs (torch.Tensor): N * C * W * H image tensors
        outputs (torch.Tensor): N * A Q(s,a) values.
        k (float, optional): The taget Lipschitz constant. Defaults to 1.0.
        mode (str, optional): If `norm_sum` it computes norm(1.0 @ Jyx).
            If `avg_norm` it computes norm(Jy_ix), the norm of the gradient
            of a single action. Ideally this second option would have
            computed the average or the sum of the gradient norms of individual
            actions, Sum_i [norm(dy_i / dx)] but it's expensive so we will take
            the expectation over the entire data. Defaults to "norm_sum".

        TLDR:
            - `norm_sum`: norm(1.0 @ dy/dx)
            - `avg_norm`: E_y [norm(dy_i/dx)], where i is random

    Returns:
        torch.Tensor: The gradient penalty term, a scalar.
    """
    N, act_num = outputs.shape
    dummy = torch.ones_like(outputs)  # N * action_num

    if mode == "avg_norm":
        # select just one output per sample in batch
        idxs = torch.randint(act_num, (N, 1))
        dummy = dummy.gather(1, idxs)
        outputs = outputs.gather(1, idxs)
    ...
    dydx = torch.autograd.grad(
        ...
        retain_graph=True,
        allow_unused=True,
    )[0]  # N * C * W * H (same as the inputs)
    dydx = dydx.view(dydx.shape[0], -1)  # flatten all except first dim N
    dydx_norm = dydx.norm(2, dim=1)  # vector 2-norm of each sample in batch
    dydx_norm = dydx_norm[dydx_norm >= k]  # mask the elements with norm >= k
    return ((dydx_norm - k) ** 2).mean()  # compute the penalty, avg over batch
...
class C51PolicyEvaluation:
    ...
    def __call__(self, batch, cb=None, update=True):
        ...
        if self.grad_penalty is not None:
            grad_norm = dydx_norm_penalty(batch[0], c51_loss.qs_probs)
            loss += self.grad_penalty * grad_norm
...
class DQNPolicyEvaluation:
    ...
    def __call__(self, batch, update=True):
        ...
        batch = to_device(batch, self.device)
        batch[0] = batch[0].float().requires_grad_(True)
        ...
        if self.grad_penalty is not None:
            if isinstance(self.grad_penalty, dict):
                k, beta, mode = (
                    self.grad_penalty["k"],
                    self.grad_penalty["beta"],
                    self.grad_penalty["mode"],
                )
            else:
                k, beta = 1.0, self.grad_penalty

            grad_norm = dydx_norm_penalty(batch[0], dqn_loss.q_values, k=k, mode=mode)
            loss += beta * grad_norm
