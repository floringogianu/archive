...
def load_policy(env, ckpt_path, opt):
    ...
    agent_args["epsilon"] = opt.val_epsilon  # purely max
...
def run(opt):
    ...
    mode = "last_5"  # every_k | last_5

    if (Path(opt.out_dir) / "max_ckpt").exists():
        # sample the maximum ckpt if it exists
        ckpt_paths = [
            p
            for p in ckpt_paths
        ...
        rlog.warning("IMPORTANT! Sampling only the last five checkpoints.")
