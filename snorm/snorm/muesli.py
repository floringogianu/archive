...
def learn(env, model, mem, optimizer, opt, obs):
    ...
    return monitor.stats(), train_step_cnt, obs
...
def run(opt):
    ...
    for epoch in range(1, opt.epochs + 1):
        trn_stats, steps, obs = learn(env, model, mem, optimizer, opt, obs)
        val_stats = pvalidate(val_env, model, opt)
