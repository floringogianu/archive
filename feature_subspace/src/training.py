...
def cosine_sim_regularization(model, beta, loss):
    loss = loss.loss.mean()
    num = 0.0
    norm = 0.0
    norm1 = 0.0
    for m in model.modules():
        if isinstance(m, (nn.Conv2d, nn.Linear)):
            ...
            norm += m.weight.pow(2).sum()
            norm1 += m.weight1.pow(2).sum()
    cosine_sim = num.pow(2) / (norm * norm1)
    print(cosine_sim)
    loss += beta * cosine_sim
    return loss
        ...
        for transition in Episode(env, policy, _state=last_state):
            ...
            if replay.is_ready:
                if total_steps % update_freq == 0:
                    ...
                    cb = None
                    if "beta" in opt and opt.beta > 0.0:
                        cb = partial(
                            cosine_sim_regularization,
                            policy_evaluation.estimator.feature_extractor,
                            opt.beta,
                        )

                    loss = policy_evaluation(batch, cb=cb).detach().item()
...
def experiment_factory(opt, offline=False):
