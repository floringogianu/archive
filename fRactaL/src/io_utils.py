...
def config_to_string(
    ...
    for key, value in cfg.__dict__.items():
        if key.startswith("__") and not verbose:
            ...
            ckey = cstr(f"[bold yellow]{key}", end="") if color else key
            text += " " * indent + ckey + ": "
            if isinstance(value, Namespace):
                text += "\n" + config_to_string(value, indent + 2, color=color)
            else:
                cvalue = cstr(f"[white]{str(value)}", end="") if color else str(value)
                text += cvalue + "\n"
    return text
...
def checkpoint_agent(path, crt_step, **kwargs):
    ...
    if replay_path is not None:
        ...
        if save_every_replay:
            # when saving every replay the path name replay_xxx.gz
