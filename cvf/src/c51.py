...
from src.schedules import constant_schedule, linear_schedule
...
class C51PolicyImprovement:
    r""" C51 DQN policy improvement.
    """

    def __init__(self, estimator, epsilon, action_space):
        ...
        if isinstance(epsilon, float):
            ...
            self.epsilon = linear_schedule(*epsilon)
...
def main():
    ...
    print(
        "target p(Q(s_,a)):\n",
        get_target_distribution(
            x, rewards, mask, 0.92, net, torch.linspace(*support)
        ),
    )
