...
class ExperienceReplay:
    ...
    def save(self, path, idx=None):
        """ Snapshots the ER buffer.
            ...
            If the idx is provided it will just store on disk all the results.
        """
        root = Path(path)
        fname = "replay.gz" if idx is None else f"replay_{idx:08d}.gz"
        fpath = root / fname
        ...
        if idx is None:
            shutil.copyfile(fpath, root / "prev_replay.gz")
    ...
    def __str__(self):
        return "{}(N={}, size={}, batch={}, hlen={})".format(
...
def load_checkpoint(root, idx):
    ...
    fpaths += [
        root / "{}_ckpt.{}.gz".format(el, idx)
        for el in ["add_count", "invalid_range"]
    ]
...
class OfflineExperienceReplay:
    ...
    def reload_N(self, N=1, ckpt_idxs=None):
        ...
        with ThreadPoolExecutor(max_workers=N) as tpe:
            replay_futures = [
                tpe.submit(self._load_one, idx) for idx in ckpt_idxs
            ]
