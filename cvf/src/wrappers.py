...
def get_wrapped_atari(
    env_name, mode="train", dopamine=False, sticky_actions=True, **kwargs
):
    ...
    if dopamine:
        return dopamine_env(
            env_name.capitalize(), sticky_actions=sticky_actions
        )
    return ALE(
        env_name,
        seed,
        kwargs.get("device", torch.device("cpu")),
        sticky_action_p=0.25 if sticky_actions else 0,
