...
def get_hardware():
    ...
    return {
        "full": {"cpu": cpus, "gpu": gpus},
        "short": {"cpu": cpus_, "gpu": gpus_},
    }
...
def save_config(cfg, path):
    ...
    with open(Path(path) / "cfg.yml", "w") as outfile:
...
def load_checkpoint(path, idx=None):
    # get last checkpoint
    if idx is None:
        ckpt_paths = sorted(Path(path).glob("*.gz"))
        ckpt_paths = [p for p in ckpt_paths if "replay" not in p.stem]
        ckpt_path = ckpt_paths[-1]
        if not ckpt_path:
            raise ValueError(f"No files with `.gz` extension found in {path}.")
    else:
